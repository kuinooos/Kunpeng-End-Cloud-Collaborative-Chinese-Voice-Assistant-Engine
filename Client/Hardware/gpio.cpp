...
int GPIO::setDirection(int dir)
{
    ...
    if (!file.is_open()){
       cout << "setDirection: write failed to open file: "
            << (this->m_sPath + "direction")
            << ", errno=" << errno << ", " << strerror(errno) << endl;
       return -1;
   }
   
    // Kunpeng board convention: dir==1 => output, dir==0 => input
    if (dir)
        file << "out"; 
    else
        file << "in";  
...
int GPIO::setValue(int val)
{
    ...
    if (!file.is_open()){
       cout << "setValue: write failed to open file: "
            << (this->m_sPath + "value")
            << ", errno=" << errno << ", " << strerror(errno) << endl;
       return -1;
   }
...
int GPIO::exportGPIO(void)
{
    ...
    if (!file.is_open()){
       cout << "exportGPIO: write failed to open file: /sys/class/gpio/export"
            << ", errno=" << errno << ", " << strerror(errno) << endl;
       return -1;
   }
   file << this->m_iPin;
...
int GPIO::unexportGPIO(void)
{
    ...
    if (!file.is_open()){
       cout << "unexportGPIO: write failed to open file: /sys/class/gpio/unexport"
            << ", errno=" << errno << ", " << strerror(errno) << endl;
       return -1;
   }
   file << this->m_iPin;
