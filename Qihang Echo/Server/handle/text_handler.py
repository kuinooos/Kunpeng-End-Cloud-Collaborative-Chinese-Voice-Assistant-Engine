    ...
    async def handle_text_message(self, data: dict):
        ...
        if data.get('type') == 'hello':
            ...
            if data.get('state') == 'idle':
                ...
                # 客户端主动通知正在播放语音（仅作记录和同步），服务端设置播放标志
                logger.info("Client is playing (TTS playback)")
                self.service_manager.is_playing = True

            elif data.get('state') == 'speaking':
                # 当客户端开始说话时，优先打断正在播放的 TTS，重置 VAD/ASR 并允许接收音频
                logger.info("Client is speaking")
                try:
                    # 停止本地正在播放的 TTS（如果有），优先让用户讲话被识别
                    self.service_manager.tts_service.tts_close()
                except Exception:
                    pass
                # 允许接收新的语音输入
                self.service_manager.is_vad = False
